...
export default function CustomCursor() {
  ...
  return (
    <>
      ...
      >
        <div className="w-2 h-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#CC0000]" />
      </motion.div>

      <motion.div
        className="fixed top-0 left-0 z-[9998] pointer-events-none"
        style={{ x: ringX, y: ringY }}
        animate={{ opacity: visible ? 1 : 0, scale: clicking ? 0.75 : hovering ? 1.6 : 1 }}
        transition={{ scale: { type: "spring", stiffness: 350, damping: 22 }, opacity: { duration: 0.15 } }}
      >
        <div className={`w-9 h-9 -translate-x-1/2 -translate-y-1/2 rounded-full border transition-colors duration-150 ${
          hovering ? "border-[#CC0000] bg-[#CC0000]/10" : "border-gray-400/40 bg-transparent"
        }`} />
      </motion.div>
    </>
  );
