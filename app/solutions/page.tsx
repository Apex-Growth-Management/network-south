          ...
          {solutions.map((cat) => (
            <div key={cat.category}>
              <div className="flex items-center gap-4 mb-10">
                ...
                <span className="text-blue-400 text-sm font-semibold tracking-widest uppercase">{cat.category}</span>
                <div className="h-px flex-1 bg-white/10" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {cat.items.map((item) => (
                  <div key={item.title} className="bg-white/[0.03] border border-white/10 rounded-2xl p-8 hover:border-blue-500/30 transition-all">
