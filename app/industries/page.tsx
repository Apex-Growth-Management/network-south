      ...
      <section className="py-24 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 gap-8">
          {industries.map((ind, i) => (
            <div
              key={ind.title}
              className={`grid md:grid-cols-2 gap-10 items-center p-8 rounded-2xl border border-white/10 bg-white/[0.02] hover:border-[#CC0000]/30 transition-all ${
                i % 2 === 1 ? "md:flex-row-reverse" : ""
              }`}
            >
              <div>
                <div className="text-4xl mb-4">{ind.icon}</div>
                <h2 className="text-2xl font-bold mb-4 text-[#ff4444]">{ind.title}</h2>
                <p className="text-white/60 leading-relaxed mb-6">{ind.description}</p>
                <Link href="/support" className="text-[#CC0000] hover:text-[#ff4444] text-sm font-medium transition-colors">
